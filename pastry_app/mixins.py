...
class GuestUserRecipeMixin:
    """
    Mixin pour factoriser la gestion des recettes multi-utilisateurs (user ou invité/guest_id).
    - Permet à un utilisateur connecté d'accéder à ses propres recettes + publiques + "de base".
    - Permet à un invité d'accéder aux recettes publiques, de base, et à ses propres recettes privées (par guest_id).
    - Attribue la recette à user ou guest_id lors de la création.
    - Forçage : les recettes créées par un invité sont privées par défaut, sauf si "public" est explicitement demandé.
    """
